...
type SizeKind = "apparel" | "waist" | "shoe" | "onesize";

const OPTION_DEFS: Record<
  Exclude<SizeKind, never>,
  { title: string; values: string[] }
> = {
  apparel: { title: "Size", values: ["XS", "S", "M", "L", "XL", "One Size"] },
  onesize: { title: "Size", values: ["XS", "S", "M", "L", "XL", "One Size"] },
  waist: { title: "Waist", values: ["28", "30", "32", "34", "36"] },
  shoe: { title: "Shoe size", values: ["40", "41", "42", "43", "44", "45"] },
};
...
const sizesFor = (p: ProductSeed): string[] => {
  if (p.sizes) return p.sizes;
  switch (p.sizeKind) {
    case "apparel":
      return APPAREL;
    case "waist":
      return OPTION_DEFS.waist.values;
    case "shoe":
      return OPTION_DEFS.shoe.values;
    case "onesize":
      return ["One Size"];
  }
};

export default async function rebuildProductsWithOptions({
  container,
}: {
  ...
  // ——— Delete our products so they can be recreated against shared options ———
  ...
  if (oldProducts.length) {
    logger.info(`Deleting ${oldProducts.length} products for rebuild...`);
  ...
  if (standaloneOptions.length) {
    ...
    await deleteProductOptionsWorkflow(container).run({
      input: { ids: standaloneOptions.map((o) => o.id) },
    });
  }

  logger.info("Creating standalone options: Size, Waist, Shoe size...");
  const { result: createdOptions } = await createProductOptionsWorkflow(
    container
  ).run({
    input: {
      product_options: [
        { title: "Size", values: OPTION_DEFS.apparel.values },
        { title: "Waist", values: OPTION_DEFS.waist.values },
        { title: "Shoe size", values: OPTION_DEFS.shoe.values },
      ],
    },
  });
  const optionByTitle = new Map(createdOptions.map((o) => [o.title, o.id]));

  // ——— Categories & collections (already created by the premium seed) ———
  ...
  // ——— Recreate products referencing the shared options ———
  logger.info(`Creating ${PRODUCTS.length} products...`);
  await createProductsWorkflow(container).run({
    input: {
      products: PRODUCTS.map((p) => {
        const optionTitle = OPTION_DEFS[p.sizeKind].title;
        const optionId = optionByTitle.get(optionTitle)!;
  ...
  logger.info(
    `Done. ${PRODUCTS.length} products rebuilt, ${missing.length} inventory levels added.`
  );
}
